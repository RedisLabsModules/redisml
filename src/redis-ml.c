...
/*
 * Create / Override a TF model
 * ml.tf.set <Id> <intercept> <coefficients ...>
 */
int TFSetCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
	if (argc < 4) {
		return RedisModule_WrongArity(ctx);
	}
	RedisModule_AutoMemory(ctx);
...
/*Run TF for a feature vector
 * ml.tf.predict <id> <features ...>
 */
int TFPredictCommand(RedisModuleCtx *ctx, RedisModuleString **argv,
		int argc) {
	if (argc < 3) {
		return RedisModule_WrongArity(ctx);
	}
...
int RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
	...
	/*Register TF data type and commands*/
	if (TFTypeRegister(ctx) == REDISMODULE_ERR) return REDISMODULE_ERR;

	RMUtil_RegisterWriteCmd(ctx, "ml.tf.set", TFSetCommand);
	RMUtil_RegisterReadCmd(ctx, "ml.tf.predict", TFPredictCommand);
