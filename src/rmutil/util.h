...
/// make sure the response is not NULL or an error, and if it is sends the error
/// to the client and exit the current function
...
#define __rmutil_register_cmd(ctx, cmd, f, mode)               \
  if (RedisModule_CreateCommand(ctx, cmd, f, mode, 1, 1, 1) == \
      REDISMODULE_ERR)                                         \
  return REDISMODULE_ERR
...
#define RMUtil_RegisterWriteCmd(ctx, cmd, f) \
  __rmutil_register_cmd(ctx, cmd, f, "write")

/* RedisModule utilities. */

/** Return the offset of an arg if it exists in the arg list, or 0 if it's not
 * there */
int RMUtil_ArgExists(const char *arg, RedisModuleString **argv, int argc,
                     int offset);

/**
Automatically conver the arg list to corresponding variable pointers according
to a given format.
You pass it the command arg list and count, the starting offset, a parsing
format, and pointers to the variables.
...
*/
int RMUtil_ParseArgs(RedisModuleString **argv, int argc, int offset,
                     const char *fmt, ...);

/**
Same as RMUtil_ParseArgs, but only parses the arguments after `token`, if it was
found.
This is useful for optional stuff like [LIMIT [offset] [limit]]
*/
int RMUtil_ParseArgsAfter(const char *token, RedisModuleString **argv, int argc,
                          const char *fmt, ...);

int rmutil_vparseArgs(RedisModuleString **argv, int argc, int offset,
...
/**
* Get a double value from an info object. Returns 1 if the value was found and
* is
